...
apply_patch()
{
	...
	then
		cp -r $YOCTO_DIR/Thor96_L5_4_47_Rel_3_1_patches/meta-quectel-community/ .
		if [ $? -ne 0 ]
		then
			echo "###################################################################################"
			echo "${red}Error during apply the patch in meta-quectel-community"
			echo "Please verify Thor96_L5_4_47_Rel_3_1_patches directory${reset}"
			echo "###################################################################################"
			exit 1
		fi
	fi
	...
	git apply --check -R $YOCTO_DIR/Thor96_L5_4_47_Rel_3_1_patches/meta-imx-patch/00* 2>/dev/null
	if [ $? -ne 0 ]
	then
		echo "###################################################################################"
		...
		git am --whitespace=fix $YOCTO_DIR/Thor96_L5_4_47_Rel_3_1_patches/meta-imx-patch/00*
		if [ $? -ne 0 ]
		then
			echo "###################################################################################"
	...
	git apply --check -R $YOCTO_DIR/Thor96_L5_4_47_Rel_3_1_patches/meta-qt5-patch/00* 2>/dev/null
	if [ $? -ne 0 ]
	then
		echo "###################################################################################"
		...
		git am --whitespace=fix $YOCTO_DIR/Thor96_L5_4_47_Rel_3_1_patches/meta-qt5-patch/00*
		if [ $? -ne 0 ]
		then
			echo "###################################################################################"
	...
	git apply --check -R $YOCTO_DIR/Thor96_L5_4_47_Rel_3_1_patches/meta-openembedded-patch/00* 2>/dev/null
	if [ $? -ne 0 ]
	then
		echo "###################################################################################"
		...
		git am --whitespace=fix $YOCTO_DIR/Thor96_L5_4_47_Rel_3_1_patches/meta-openembedded-patch/00*
		if [ $? -ne 0 ]
		then
			echo "###################################################################################"
	...
	git apply --check -R $YOCTO_DIR/Thor96_L5_4_47_Rel_3_1_patches/meta-browser-patch/00* 2>/dev/null
	if [ $? -ne 0 ]
	then
		echo "###################################################################################"
		...
		git am --whitespace=fix $YOCTO_DIR/Thor96_L5_4_47_Rel_3_1_patches/meta-browser-patch/00*
		if [ $? -ne 0 ]
		then
			echo "###################################################################################"
